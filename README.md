...
- TDD를 적용하면서 기능 개발 전에 테스트 코드를 먼저 작성한 뒤, 구현부를 완성해 나가는 방식으로 안정성을 확보했습니다. 특히 TodoData 상태 전환, 팀 생성·초대 로직, 승인·반려 처리 등 오류가 발생하기 쉬운 핵심 로직에 대해 JUnit 기반 테스트케이스를 생성하여 반복적으로 검증했습니다.
